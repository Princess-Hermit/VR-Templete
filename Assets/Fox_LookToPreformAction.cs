...
public class Fox_LookToPreformAction : MonoBehaviour
{
	...
	void Update ()
	{
		...
		if(timer != 0 && cur_time - timer == activateTimeSec)
		{
			...
			//selectableObject.SetActive (false); // Example Action
			//Change the target of the Foxes
			//for (int i = 0; i < foxes.Length; i++)
			//{
			fox.GetComponent<MalbersAnimations.AnimalAIControl>().target = this.transform;
			selectableObject.GetComponent<AudioSource> ().Play();
			//}
		}
